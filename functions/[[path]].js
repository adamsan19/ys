...
export async function onRequest(context) {
  ...
  // 1. Cek dulu apakah ada file statis (JSON/HTML) yang cocok?
  // Di Pages, file statis memiliki prioritas tinggi, tapi untuk memastikan
  // kita tidak mendouble fetch, kita biarkan Pages handle static secara default.
  // Kode ini hanya handle route dinamis.
  ...
  if (p.startsWith("/e/")) return withCache(request, () => detail(url));
  if (p.startsWith("/f/")) return withCache(request, () => search(url));
  if (p.startsWith("/list/")) return withCache(request, () => list(url));

  // Jika tidak ada yang cocok, teruskan ke file statis atau 404
  return next();
}
